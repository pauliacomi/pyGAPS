...
class TestAlphaSPlot():
    ...
    def test_alphas(self, sample):
        ...
        if sample.get('as_area', None):
            ...
            ref_filepath = os.path.join(DATA_N77_PATH, DATA[sample['as_ref']]['file'])
            ...
            assert isclose(results[-1].get('adsorbed_volume'),
                           sample['as_volume'], err_relative, err_absolute_area)
            assert isclose(results[0].get('area'),
                           sample['as_area'], err_relative, err_absolute_volume)
    ...
    def test_alphas_choice(self):
        ...
        assert isclose(results[-1].get('area'),
                       sample['s_as_area'], err_relative, err_absolute_volume)
